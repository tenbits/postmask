...
var OptimizerCtx = /** @class */ (function () {
    function OptimizerCtx(report, filename, source) {
        this.report = report;
        this.filename = filename;
        this.source = source;
    }
    OptimizerCtx.prototype.error = function (text, node) {
        this.write(text, node, 'error');
    };
    OptimizerCtx.prototype.info = function (text, node) {
        this.write(text, node, 'info');
    };
    OptimizerCtx.prototype.warn = function (text, node) {
        this.write(text, node, 'warn');
    };
    OptimizerCtx.prototype.write = function (text, node, level) {
        if (node === void 0) { node = null; }
        var message = new Result_1.Message;
        ...
        if (node && node.sourceIndex) {
            var before = this.source.substring(0, node.sourceIndex);
...
exports.OptimizerCtx = OptimizerCtx;
var OptimizerCtor = /** @class */ (function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        ctx = new OptimizerCtx(result.report, path, source);
     ...
     * @param fn
     */
    OptimizerCtor.prototype.registerOptimizer = function (pattern, fn, middleware) {
        var name = pattern;
    ...
    OptimizerCtor.prototype.processTags = function (root, ctx) {
        ...
        global_1.mask.TreeWalker.walkAsync(root, function (node, next) {
            ...
            if (fns != null) {
                _this.processOptimizers(node, ctx, fns).then(next);
                return;
            }
            next();
        }, function (root) {
...
function prepare(path, options) {
    ...
    options.plugins.map(function (name) {
        ...
        var config = options.configs && options.configs[name];
        if (config) {
            global_1.mask.cfg(name, config);
        }
        plugin.initialize(Optimizer_1.default, config, global_1.mask, global_1.io);
        plugins.push(plugin);
    });
